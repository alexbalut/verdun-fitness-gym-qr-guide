...
Seed gym: `verdun-fitness`, `primaryColor: #FF9800`, `secondaryColor: #000000`.

## Multi-tenant gym

Admin session carries `gymId`. APIs scope mutations to that gym. Member home loads slug `verdun-fitness` (fallback: first gym).
...
| Workout + history | localStorage keys `verdun-fitness-workout:v1:<slug>` and `verdun-fitness-workout-history:v1:<slug>` |
