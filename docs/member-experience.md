...
Members never log in. `/` is the in-product gym home for **Verdun Fitness**.
...
3. Friendly URLs: `/m/verdun-fitness/lat-pulldown`.
...
Keyed by gym slug (`verdun-fitness`). No member account.

| Key | Contents |
|-----|----------|
| `verdun-fitness-workout:v1:<slug>` | In-progress session JSON |
| `verdun-fitness-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
